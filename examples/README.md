...
- [`sample-corpus.md`](sample-corpus.md): a tiny synthetic corpus (a real one would be 20,000+ words). Shows the raw, multi-register input the builder expects. It includes both a pre-2023 written bank and a 2025-2026 spoken (dictation) bank, to demonstrate the two-bank model: speech is clean at any date because it cannot be AI-edited in the moment. In a real run this becomes `corpus.md` inside the bundle, the cleaned and register-sorted source. It is the ninth file, kept one level up here so the example reads cleanly.
